-- Mirrors migrations_pg/046_run_idempotency.sql (Postgres).
...
CREATE TABLE IF NOT EXISTS run_idempotency (
    ...
    --
    -- ON DELETE CASCADE is the entire retention story (see the header): a claim
    -- lives exactly as long as the run it names. SQLite enforces the cascade
    -- only with `PRAGMA foreign_keys = ON`, which the pool sets on every
    -- connection at init (crates/dagron-core/src/db/sqlite.rs); without it a
    -- purged run would leave an orphaned claim to replay to a vanished run_id.
